function [v,aP,aE,aW,aN,aS,Su] = update_v_momentum(Lx,Ly,u,ue,uw,v,vn,vs,M,N,mu,rho,p,alpha_u)
...
x(1:M-1,1) = dx/2 + [1:M-1] * dx;

y(1,1:N-1) = dy/2 + [1:N-1] * dy;
...
Su = zeros(M,N) ;
for j = 2:N-1 
    Su(1,j) = (2*Dy + max(0,-Fn(i,j)))*v(1,j) + 2 * dx * (p(2,j) - p(1,j)) / aP(i,j);
end 
for j = 2:N-1 
    Su(M,j) = (Dx + max(Fs(i,j),0))*v(M,j) + 2 * dx * (p(M,j) - p(M-1,j)) / aP(i,j);
end 
for i = 2:M-1 
    Su(i,1) = ((2*Dx + max(Fw(i,j),0)))*v(i,1) + 2 * dx * (p(i+1,1) - p(i-1,1)) / aP(i,j);
end 
for i = 2:M-1 
    Su(i,N) = (2*Dx + max(0,-Fe(i,j)))*v(i,N) + 2 * dx * (p(i+1,N) - p(i-1,N)) / aP(i,j);
end
for i = 2:M-1
    for j = 2:N-1
        Su(i,j) = aP(i,j) / alpha_u * (1-alpha_u) * v(i,j) + 2 * dx * (p(i+1,j) - p(i-1,j)) / aP(i,j);
    end
end
